...
export function searchMovies() {
    ...
    console.log("%cBúsqueda de películas con el término:", "color: #2196F3", searchTerm);

    // Realizar la solicitud a la API de OMDB
    const apiKey = "ab8939f0";
    fetch(
        `https://www.omdbapi.com/?apikey=${apiKey}&s=${encodeURIComponent(searchTerm)}&y=&page=1&type=movie&plot=short&r=json`
    )
    .then((response) => { 
